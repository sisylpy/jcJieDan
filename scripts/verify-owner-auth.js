...
const loginSource = fs.readFileSync(path.join(root, 'subPackage-auth/pages/login/login.js'), 'utf8')
...
if (!onLoadSource.includes('this._login()') || onLoadSource.includes('verifiedInviteCode')
  ...
  failures.push('subPackage-auth/pages/login/login.js: 现有老板必须先换取 Owner Token，邀请码只能拦截新用户注册')
}
