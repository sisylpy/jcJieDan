...
Page({
  ...
  toEditApplyDep(e) {
    ...
    if (itemStatus == 4) {
      wx.showModal({
        ...
        confirmText: "追加补货",
        cancelText: "知道了",
        success: (res) => { // 用箭头函数保证 this 指向 Page
          if (res.cancel) {
            // 取消按钮"知道了"，只关闭弹窗，不做其他操作
            this.setData({
              showDep: false
            });
          } else if (res.confirm) {
  ...
  _saveOrder: function (e) {
   ...
   if(this.data.userInfo !== null){
    ...
    console.log("iishhshehhehehehhehhehhehhehheehheh")
    ...
    then(res => {
      if (res.result.code == 0) {
        ...
        if (this.data.tab1Index == 1) {
          const nxDistributerGoodsId = this.data.depGoods.nxDistributerGoodsId;
          const goodsList = this.data.goodsList || [];
          for (let i = 0; i < goodsList.length; i++) {
            if (goodsList[i].nxDistributerGoodsId === nxDistributerGoodsId) {
              const updatePath = `goodsList[${i}].nxDistributerStandardEntities`;
              this.setData({
                [updatePath]: standardArr
              });
              break;
            }
          }
  ...
  // 对比数据库商品和接口返回商品的差异
  // compareGoodsWithDatabase() {
  //   // 你的标准对比数据
  //   const databaseGoods = [
  //     "香菜", "罗马生菜", "法香", "苏子叶", "绿尖椒", "圆生菜", "紫甘蓝", "切面条", "鲜口蘑", "紫皮洋葱",
  //     "西葫芦", "白萝卜", "大蒜米", "菠菜", "大葱", "鸡蛋", "黄瓜", "土豆", "美人椒", "大米",
  //     "红皮萝卜", "香菇", "杏鲍菇", "小米", "柠檬 黄", "红小米辣椒", "香葱", "冬瓜", "胡萝卜", "黄皮洋葱",
  //     "红薯", "豆腐", "香叶(九层塔)", "韩苏", "绿豆芽", "樱桃小萝卜", "长豇豆", "杭椒", "金瓜", "芒果",
  //     "橙子", "哈密瓜", "龙眼蒜米", "酸奶", "黄豆", "鲜猪五花肉", "苹果", "火碱", "鲜猪肉馅", "冷面",
  //     "辣鲜露", "雪里红", "香油", "熊猫炼乳", "虾皮", "干黑木耳", "午餐肉", "干腐竹", "东古一品鲜", "水晶粉丝",
  //     "立白洗洁精", "酒精快", "40-50冷冻大虾", "锡纸", "八角", "保鲜膜", "海天草菇老抽", "桂皮", "鲜猪棒骨", "鲜带皮鸡腿肉",
  //     "橡胶手套", "酸菜丝", "白芝麻", "小蒜米", "1000餐盒", "花边纸", "皮围裙", "网帽", "打印纸", "玉米粒",
  //     "食品袋", "裙带菜", "高粱米", "鲜花椒", "大号垃圾袋", "鱼酸菜", "孜然粒", "黄油", "苹果醋", "沙拉汁",
  //     "皇冠梨", "熟白芝麻", "大白梨", "金枪鱼块", "价格签", "料理酒", "芝士碎", "冰激淋勺", "中号垃圾袋", "金枪鱼罐头",
  //     "挤瓶", "火锅年糕", "除胶剂", "上海白醋", "去皮鸡腿肉", "丁晴手套", "保鲜膜 小号", "腔骨", "料桶", "卫生纸",
  //     "消毒液", "擦手纸", "黑方餐盒 500号", "金龙鱼油 非转基因", "电子打火枪", "陈皮", "辛拉面", "连体盒P4", "拌饭海苔碎", "沙拉酱  香甜",
  //     "蒜味薯角", "牛肉粉", "餐巾纸", "洗洁精", "水龙头", "硅胶刮板", "不粘平底锅", "海天草菇老抽", "照烧酱", "天妇罗粉",
  //     "线手套", "蟹棒", "口罩", "干白菜", "黑松露酱", "换气瓶 东元", "下水管", "拌饭素  丰滋雅", "四号电池", "一次性手套",
  //     "不锈钢锅", "玉米粒罐头", "海天老抽", "藤椒 鲜", "海盐", "榴莲酱", "彩色朱古针", "口屏", "圆珠笔", "夹子"
  //   ];
  
  //   // 页面实际数据
  //   const apiGoods = this.data.depGoodsArrAi.map(item => item.nxDdgDepGoodsName);
  
  //   // 只关心“对比数据”多出来的
  //   const missingGoods = databaseGoods.filter(goods => !apiGoods.includes(goods));
  
  //   console.log('=== 对比数据中页面没有的商品 ===');
  //   if (missingGoods.length > 0) {
  //     missingGoods.forEach((goods, index) => {
  //       console.log(`${index + 1}. ${goods}`);
  //     });
  //   } else {
  //     console.log('页面数据已包含所有对比数据商品');
  //   }
  // }
