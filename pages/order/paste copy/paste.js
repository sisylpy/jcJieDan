...
Page({
  ...
  onLoad: function (options) {
    ...
    console.log("========== onLoad 窗口尺寸 ==========");
    console.log("globalData.windowWidth:", globalData.windowWidth);
    console.log("globalData.rpxR:", globalData.rpxR);
    console.log("计算后的 windowWidth:", windowWidth);
    console.log("搜索结果列表宽度 (windowWidth - 40):", windowWidth - 40);
    console.log("商品名称输入框宽度 (旧):", (windowWidth - 180) / 2);
    console.log("商品名称输入框宽度 (新):", goodsNameWidth);
    console.log("数量输入框宽度 (新):", quantityWidth);
    console.log("规格输入框宽度 (新):", standardWidth);
    console.log("父容器宽度 (新):", containerWidth);
    ...
    if (pasteDepList) {
      ...
      for (var i = 0; i < pasteDepList.length; i++) {
        ...
        if (pDepId == this.data.depId) {
          console.log("找到匹配的 depId，加载订单数据，订单数量:", pasteDepList[i].arr ? pasteDepList[i].arr.length : 0);
          // 只在开发调试时输出详细订单列表（减少日志输出）
          // if (pasteDepList[i].arr && pasteDepList[i].arr.length > 0) {
          //   pasteDepList[i].arr.forEach((order, idx) => {
          //     console.log(`订单 ${idx}:`, "名称:", order.nxDoGoodsName, "status:", order.nxDoStatus);
          //   });
          // }
          // 从缓存恢复订单数据
  ...
  _finishClearSave(filteredOrderArr, deletedOrders) {
    ...
    if (filteredOrderArr.length === 0) {
      ...
      // 从缓存中读取最新的 pasteDepList
      var pasteDepList = wx.getStorageSync('pasteDepList');
      // 从 pasteDepList 中删除当前部门的数据
      var updatedPasteDepList = pasteDepList ? pasteDepList.filter(item => item.depId !== this.data.depId) : [];
      ...
      // 重置相关状态，确保 orderArr 也被清空
      this.setData({
        orderArr: [], // 确保 orderArr 被清空
        pasteDepList: updatedPasteDepList.length > 0 ? updatedPasteDepList : null,
        pasteDepId: null,
        pasteDep: null,
        pasteDepIndex: -1,
        saveCount: null, // 重置保存计数
