...
const viewBarHeight = 50; // px，与出库工作区顶部工具栏一致
...
module.exports = function purchaseComponent() {
  return {

  properties: {
    // 由「库存」页内嵌使用时传 true：隐藏自带导航栏、一级页签和页面内底栏
    embedded: {
      type: Boolean,
      value: false
    }
  ...
  data:{
    ...
    purchaseWorkspaceHeight: 0,
},

  observers: {
    selectedArr(selectedArr) {
      if (this.data.embedded) {
        this.triggerEvent('selectionchange', { count: (selectedArr || []).length });
      }
    }
  },
  ...
  methods: {
    _onPageShow() {
      ...
      // 外采工作区已取消内部“未采购 / 采购中”页签，列表直接占用原页签高度。
      const contentHeight = (screenHeight - navBarHeight - tabBarHeight) * rpxRatio - mergedWorkBarHeight;
      const purchaseAppEntryBarHeight = viewBarHeight * rpxRatio;
      const purchaseWorkspaceHeight = Math.max(
        0,
        // pt-20 在全局样式中是 20rpx 的顶部间距，也要从列表高度中扣除。
        contentHeight - (this.data.embedded ? purchaseAppEntryBarHeight + 20 : 0)
      );
      this.setData({ 
        contentHeight: contentHeight,
        purchaseAppEntryBarHeight: purchaseAppEntryBarHeight,
        purchaseWorkspaceHeight: purchaseWorkspaceHeight,
    ...
    _visibleTabBar() {
      var nativeTabBar = typeof this.getTabBar === 'function' ? this.getTabBar() : null;
      return nativeTabBar || this.selectComponent('#local-tab-bar');
    },

    _updateVisibleTabBar(data, refresh, retry) {
      var bar = this._visibleTabBar();
      if (!bar) {
        if ((retry || 0) < 3) {
          setTimeout(() => this._updateVisibleTabBar(data, refresh, (retry || 0) + 1), 0);
        }
        return;
      }
      if (refresh && typeof bar.refreshTabs === 'function') bar.refreshTabs();
      bar.setData(data || {});
    },
    ...
    openPurchaseApp() {
      ...
      var disInfo = userInfo.nxDistributerEntity || {};
      var disId = Number(wx.getStorageSync('ownerDistributerId') || this.data.disId || disInfo.nxDistributerId || 0);
      var purchaserId = Number(wx.getStorageSync('ownerUserId') || userInfo.nxDistributerUserId || 0);
      if (!(disId > 0) || !(purchaserId > 0)) {
        wx.showToast({ title: '采购身份信息不完整', icon: 'none' });
        return;
      }
      var path = 'pages/workbenchV2/workbenchV2?nxDisId=' + encodeURIComponent(disId) +
        '&nxDisPurUserId=' + encodeURIComponent(purchaserId) +
        '&from=nx';
