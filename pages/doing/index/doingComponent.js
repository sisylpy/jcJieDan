...
module.exports = function doingComponent() {
  return {
  ...
  methods: {
    onEmbeddedWorkspaceTouchStart(e) {
      if (!this.data.embedded || this.data.innerCurrent !== 0) return;
      var touch = e.touches && e.touches[0];
      if (!touch) return;
      this._workspaceTouchStart = {
        x: touch.clientX,
        y: touch.clientY
      };
    },

    onEmbeddedWorkspaceTouchEnd(e) {
      if (!this.data.embedded || this.data.innerCurrent !== 0 || !this._workspaceTouchStart) return;
      var touch = e.changedTouches && e.changedTouches[0];
      var start = this._workspaceTouchStart;
      this._workspaceTouchStart = null;
      if (!touch) return;

      var deltaX = touch.clientX - start.x;
      var deltaY = touch.clientY - start.y;
      if (deltaX >= 60 && Math.abs(deltaX) > Math.abs(deltaY) * 1.5) {
        this.triggerEvent('workspaceback');
      }
    },

    _onPageShow() {
      // 协作伙伴作为第 2 个底部入口的内部页面，使用页面内挂载的同款底栏。
      ...
      const contentHeight = (screenHeight - navBarHeight - tabBarHeight - viewBarHeight) * rpxRatio - mergedWorkBarHeight;
