...
import {
  ...
  updateDepUserAdmin,
  updateGroupName,
  updateGroupNameWithNxDisBusiness,
  getGbDisDepInfo
}
...
Page({
  ...
  onLoad: function (options) {
    ...
    getGbDisDepInfo(options.gbDisId).then(res => {
      if (res.result.code == 0) {
        var depInfoValue = res.result.data;
        this.setData({
  ...
  updateDepInfo() {
    updateGroupNameWithNxDisBusiness(this.data.depInfo).then(res => {
      if (res.result.code == 0) {
        wx.showToast({
          title: '修改成功',
        })
