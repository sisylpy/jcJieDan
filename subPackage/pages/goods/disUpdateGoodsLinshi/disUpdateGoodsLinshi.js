...
import { disGoodsUpdate } from '../../../../lib/apiDistributer';
...
Page({
  ...
  onDetailInput: function(e) {
    this.setData({
      'editItem.nxDgGoodsDeatil': e.detail.value,
      imgChanged: true
    });
  },
  ...
  if (!that.data.imgChanged && 
      (!that.data.editItem.nxDgGoodsStandardname || that.data.editItem.nxDgGoodsStandardname === '') && 
      (!that.data.editItem.nxDgGoodsDeatil || that.data.editItem.nxDgGoodsDeatil === '')) {
    wx.showToast({
      title: '请先进行修改',
      icon: 'none'
    });
