...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      mode: options.id ? 'edit' : 'add'
    });
    this.loadScopeCategories();
    if (this.data.mode === 'edit' && options.id) {
      this.loadCoupon(Number(options.id));
    } else {
  ...
  loadScopeCategories() {
    ...
    getDisGoodsCataWithCount({ disId: this.data.disId, goodsType: 99 })
      .then(res => {
        ...
        this._scopeCataReady = true;
        const scopeType = this.data.formData.scopeType;
        if (scopeType === 'CATEGORY') {
          this.setData({
            scopeOptions: markSelectedOptions(flattenDisCategories(list), this.data.selectedScopeIds)
          });
          this.syncCategoryScopeItems();
        }
        if (this._pendingCategoryInit) {
          this._pendingCategoryInit = false;
  ...
  initCategoryOptions() {
    if (!this._scopeCataReady) {
      this._pendingCategoryInit = true;
      return;
    }
    this.setData({
      scopeOptions: markSelectedOptions(flattenDisCategories(this._scopeCata || []), this.data.selectedScopeIds)
    });
    this.syncCategoryScopeItems();
  },
  ...
  loadCoupon(id) {
    ...
    getDistributerCouponList(this.data.disId)
      .then(res => {
        ...
        const item = rows.find(r => r.nxDistributerCouponId === id);
        if (!item) {
          wx.showToast({ title: '未找到优惠券', icon: 'none' });
          return;
        }
        ...
        this.setData({
          ...
          audienceIndex: Math.max(0, AUDIENCE_TYPES.indexOf(item.audienceType || 'SHOP_CUSTOMER')),
          ...
          formData: {
            ...
            audienceType: item.audienceType || 'SHOP_CUSTOMER',
          ...
          if (scopeType === 'GOODS') {
            ...
            this.setData({ selectedGoods: this.data.selectedScopeItems.slice() });
          }
        });
      })
  ...
  resetScopeSelection(scopeType) {
    scopeType = scopeType || this.data.formData.scopeType;
    this.setData({
      selectedScopeIds: [],
      selectedScopeItems: [],
      selectedGoods: [],
      scopeOptions: scopeType === 'CATEGORY'
        ? markSelectedOptions(flattenDisCategories(this._scopeCata || []), [])
        : []
    });
  },
  ...
  buildPreviewCoupon(source) {
    ...
    const scopeTypeText = SCOPE_TYPE_TEXTS[SCOPE_TYPES.indexOf(item.scopeType)] || '全店通用';
    let descText = '';
    if (item.nxDcType === 1) {
      const percent = item.nxDcPricePercent || '0';
      const percentNum = Number(percent);
      const percentLabel = (!isNaN(percentNum) && percentNum > 10) ? (percentNum / 10) + '折' : percent + '折';
      descText = '满' + (item.nxDcSubtotalPrice || '0') + ' 享' + percentLabel;
    } else {
      descText = '满' + (item.nxDcSubtotalPrice || '0') + ' 减' + (item.nxDcPrice || '0') + '元';
    }
    let validText = '长期有效';
    if (item.nxDcStartDate && item.nxDcStopDate) {
      validText = item.nxDcStartDate.replace(/-/g, '.') + '—' + item.nxDcStopDate.replace(/-/g, '.');
    } else if (item.nxDcStartDate) {
      validText = item.nxDcStartDate + ' 起可用';
    } else if (item.nxDcStopDate) {
      validText = '有效期至 ' + item.nxDcStopDate;
    }
    const amountText = item.nxDcType === 1
      ? (function () { const n = Number(item.nxDcPricePercent || '0'); return (!isNaN(n) && n > 10) ? (n / 10) + '折' : item.nxDcPricePercent + '折'; })()
      : '¥' + (item.nxDcPrice || '0');
    return {
      previewName: item.nxDistributerCouponName || '优惠券名称',
      previewAmountText: amountText,
      previewThresholdText: '满' + (item.nxDcSubtotalPrice || '0') + '元可用',
      previewValidText: validText,
      previewScopeText: scopeTypeText,
      previewDescText: descText
    };
  },
  ...
  bindScopeTypeChange(e) {
    ...
    this.setData({
      ...
      this.resetScopeSelection(scopeType);
    });
  },
  ...
  bindAudienceChange(e) {
    const i = Number(e.detail.value);
    this.setData({ audienceIndex: i, 'formData.audienceType': AUDIENCE_TYPES[i] });
  },
  ...
  openGoodsPicker() {
    wx.navigateTo({
      ...
      success: (res) => {
        res.eventChannel.emit('initSelected', {
          selectedIds: this.data.selectedScopeIds,
          selectedItems: this.data.selectedScopeItems
        });
      }
    });
  },

  removeSelectedGoods(e) {
    const id = Number(e.currentTarget.dataset.id);
    let ids = (this.data.selectedScopeIds || []).slice();
    let items = (this.data.selectedScopeItems || []).slice();
    const idx = ids.indexOf(id);
    ...
    api(f)
      .then(res => {
        ...
        if (res.result.code === 0) {
          ...
          if (prev && prev.fetchList) prev.fetchList();
