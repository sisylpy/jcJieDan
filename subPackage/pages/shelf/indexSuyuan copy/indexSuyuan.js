...
import {
  ...
  disSavePurGoodsSaveStockSunHola,
  updateDisStockSunHola,
  updateDisStockSunHolaWithFile,
  disSavePurGoodsSaveStockWithTraceReport,
...
Page({
  ...
  // 根据业务类型选择接口
  _getShelfGoodsApi(shelfId, page = 1, limit = 15) {
    if (this.data.disInfo && this.data.disInfo.nxDistributerType == 2) {
      return getShelfGoodsWithTraceReport(shelfId, page, limit);
    } else {
      return getShelfGoods(shelfId, page, limit);
    }
  },
  ...
  updateShelfGoodsData(e, keepCurrentPage = false) {
    ...
    // 否则只加载第一页
    this._getShelfGoodsApi(this.data.shelfId, 1, this.data.limit)
      .then(res => {
  ...
  _refreshShelfGoodsWithCurrentPage() {
    ...
    const loadPage = (page) => {
      return this._getShelfGoodsApi(shelfId, page, limit).then(res => {
        if (res.result.code == 0) {
          const pageData = res.result.page || {};
  ...
  changeShelfId(e) {
    ...
    this._getShelfGoodsApi(e.currentTarget.dataset.id, 1, this.data.limit)
      .then(res => {
        load.hideLoading();
  ...
  confirmInputPurStockSuyuan(e) {
    ...
    const traceReportData = {
      // 采购商品参数（必填）
      nxDpgDisGoodsId: String(purGoods.nxDpgDisGoodsId),
      nxDpgDistributerId: String(purGoods.nxDpgDistributerId),
      nxDpgQuantity: String(purGoods.nxDpgQuantity),
      nxDpgBuyPrice: String(purGoods.nxDpgBuyPrice),
      nxDpgBuySubtotal: String(purGoods.nxDpgBuySubtotal),
      // 采购商品参数（可选）
      nxDpgDisGoodsFatherId: purGoods.nxDpgDisGoodsFatherId ? String(purGoods.nxDpgDisGoodsFatherId) : '',
      ...
      nxDistributerGoodsShelfGoodsId: purGoods.nxDistributerGoodsShelfGoodsId ? String(purGoods.nxDistributerGoodsShelfGoodsId) : '',
      // 溯源报告参数（可选）
      nxTrSupplierId: item.nxTrSupplierId ? String(item.nxTrSupplierId) : '',
      nxTrSupplierName: item.nxTrSupplierName || '',
      nxTrSupplierContact: item.nxTrSupplierContact || '',
      nxTrValidStartDate: item.nxTrValidStartDate || '',
      nxTrValidEndDate: item.nxTrValidEndDate || '',
      nxTrRemark: item.nxTrRemark || ''
    };

    // 溯源报告文件路径（如果有）
    const traceReportFile = item.traceReportFile || '';
    ...
    savePromise.then(res => {
      if (res.result.code == 0) {
        ...
        // 优化：只更新列表中对应商品的库存和采购信息，不刷新整个列表
        const resultData = res.result.data || {};
        console.log('disSavePurGoodsSaveStockWithTraceReport - 接口返回数据（完整）:', JSON.stringify(resultData, null, 2));
        console.log('disSavePurGoodsSaveStockWithTraceReport - 接口返回数据（对象）:', resultData);
        
        if (goodsId) {
          // 如果接口返回了采购商品信息，更新采购信息
          if (resultData.purchaseGoods) {
            console.log('disSavePurGoodsSaveStockWithTraceReport - 更新采购信息');
            this.updatePurchaseGoodsInList(goodsId, shelfGoodsId, resultData.purchaseGoods, isUnshelf);
          }
          ...
          const hasStockList = resultData.stockEntities || resultData.nxDisGoodsShelfStockEntities || resultData.stockList;
          // 检查是否是单个库存对象（有 nxDgssDate 或 nxDistributerGoodsShelfStockId 等字段）
          const isSingleStock = resultData.stockEntity || resultData.nxDistributerGoodsShelfStockId || resultData.nxDgssDate;
          
          if (hasStockList || isSingleStock) {
            console.log('disSavePurGoodsSaveStockWithTraceReport - 更新库存信息，hasStockList:', hasStockList, 'isSingleStock:', isSingleStock);
            // 如果是单个库存对象，需要转换为列表格式
            const stockData = isSingleStock && !hasStockList ? { stockEntities: [resultData.stockEntity || resultData] } : resultData;
            this.updateStockInList(goodsId, shelfGoodsId, null, stockData, isUnshelf);
          } else {
            console.warn('disSavePurGoodsSaveStockWithTraceReport - 未找到库存批次信息，返回数据:', resultData);
          }
        }
        
        this.setData({
          showInputPurStockSuyuan: false,
          showOperation: false,
          isEditPurchase: false, // 重置修改模式
          item: null, // 清空item对象，避免下次打开弹窗时遗留数据
        });
  ...
  onScrollToLower() {
    ...
    if (isShowingUnshelfGoods) {
      ...
      const nextPage = this.data.currentPage + 1;
      this._getShelfGoodsApi(this.data.shelfId, nextPage, this.data.limit).then(res => {
        this.setData({
          isLoading: false
        });
  ...
  showStockDetail() {
    ...
    if (!shelfGoods) {
      ...
      return
    }subPackage/pages/shelf/index/index

    if(this.data.disInfo.nxDistributerType == 2){ 
      this.setData({
        showStockDetailSuyuan: true,
      })
    }else{
    this.setData({
  ...
  confirmStockDetailSuyuan(e) {
    ...
    if (hasNewImage && imagePreview) {
      updateDisStockSunHolaWithFile({
        ...
        if (res.result.code === 0) {
          ...
          const imageUrl = res.result.data?.nxDgssStockImage || nxDgssStockImage
          const component = this.selectComponent('#stockDetailModal')
          if (component && component.updateLocalData) {
            component.updateLocalData(stockId, restWeight, sellingPrice, imageUrl, nxDgssStockRemark)
    ...
    updateDisStockSunHola({
      ...
      sellingPrice: String(sellingPrice),
      nxDgssStockRemark: nxDgssStockRemark || '',
      nxDgssStockImage: nxDgssStockImage || ''
    }).then(res => {
      load.hideLoading();
      if (res.result.code === 0) {
        wx.showToast({ title: '保存成功', icon: 'success' });
