...
export const disGetUnshelfGoods = (disId, page = 1, limit = 10, shelfGoodsType) => {
  return new Promise((resolve, reject) => {
    ...
    if (shelfGoodsType !== undefined && shelfGoodsType !== null && shelfGoodsType !== '') {
      params.push('shelfGoodsType=' + shelfGoodsType);
    }
    if (params.length > 0) {
      url += '?' + params.join('&');
    }
...
export const assignSupplierPurchaseOwner = (relationId, purchaserUserId, key) =>
  inventoryBatchRequest('purchase/supplier-ownership/relations/' + relationId +
    '/responsible-purchaser/' + purchaserUserId, 'POST', {}, key)
...
export const getPurchaseManagementPendingStockIns = data => purchaseManagementRequest('pending-stock-ins', data)
