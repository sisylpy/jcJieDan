...
export const getPurchaseManagementSupplier = (id, data) => purchaseManagementRequest('suppliers/' + id, data)
export const getPurchaseManagementSupplierBatches = (id, data) => purchaseManagementRequest('suppliers/' + id + '/batches', data)
...
// ===== 阶段4供应商邀请与自动订货协同 =====
function supplierCollaborationRequest(path, method, data, idempotencyKey) {
  const header = { 'Content-Type': 'application/json' }
  if (idempotencyKey) header['Idempotency-Key'] = idempotencyKey
  return new Promise((resolve, reject) => getApp().ownerRequest({
    url: apiUrl.apiUrl + 'supplier-collaboration/' + path,
    method: method || 'GET', data: data || {}, header,
    success: res => resolve({ result: res.data }), fail: reject
  }))
}
export const createSupplierInvitation = (data,key) => supplierCollaborationRequest('invitations','POST',data,key)
export const getSupplierInvitations = data => supplierCollaborationRequest('invitations','GET',data)
export const reissueSupplierInvitation = invitationId => supplierCollaborationRequest('invitations/'+invitationId+'/reissue','POST',{})
export const getSupplierCollaborationRelations = data => supplierCollaborationRequest('relations','GET',data)
export const getSupplierCollaborationBatches = data => supplierCollaborationRequest('batches','GET',data)
export const getSupplierCollaborationBatch = id => supplierCollaborationRequest('batches/'+id,'GET')
export const actSupplierRemainingDemand = (batchId,goodsId,data,key) => supplierCollaborationRequest('batches/'+batchId+'/goods/'+goodsId+'/remaining-action','POST',data,key)
export const assignSupplierResponsiblePurchaser = (relationId,purchaserId) => new Promise((resolve,reject)=>getApp().ownerRequest({
  url:apiUrl.apiUrl+'purchase/supplier-ownership/relations/'+relationId+'/responsible-purchaser/'+purchaserId,
  method:'POST',success:res=>resolve({result:res.data}),fail:reject
}))
...
export const getPurchaseFinanceOverview = () => purchaseFinanceRequest('overview','GET')
