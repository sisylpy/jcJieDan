...
var DESKTOP_AUTH_BASE = apiUrl.apiUrl + 'desktop-dispatch/v1/'

/** 已登录老板端使用现有调度令牌批准桌面登录。 */
export const approveDesktopDispatchChallenge = function (challengeId) {
  return wrapRequest({
    url: DESKTOP_AUTH_BASE + 'auth/challenges/' + encodeURIComponent(challengeId) + '/approve',
    method: 'POST',
    dispatchAuth: true,
    data: {}
  })
...
/**
 * 老板可使用 wx.login 一次性 code 批准桌面调度登录。
 * 文员不进入配送调度模块，仅使用桌面端订单工作台。
 */
export const approveDesktopDispatchChallengeByWechat = function (challengeId, code) {
  return wrapRequest({
    url: DESKTOP_AUTH_BASE + 'auth/challenges/' + encodeURIComponent(challengeId) + '/mobile-approve',
    method: 'POST',
    ownerAuth: false,
    data: { code: code }
  })
