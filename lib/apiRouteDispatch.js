...
/**
 * 司机今日装车任务（ASSIGNED，含 plan 下该司机 route/stops）
 * GET /api/nxdisroutedispatch/driver/loading/today
 *
 * @param {Object} data
 * @param {number} data.driverUserId 司机用户 ID（nx_distributer_user）
 * @returns {Promise<{ result: Object }>} result.data = { routeDate, planId, planStatus, driverRoute, tasks }
 */
export const getDriverLoadingToday = function (data) {
  return wrapRequest({
    url: BASE + 'driver/loading/today',
    method: 'GET',
    data: {
      driverUserId: data.driverUserId
    }
...
/**
 * 司机今日配送任务（READY_TO_GO，含 plan 下该司机 route/stops）
 * GET /api/nxdisroutedispatch/driver/delivery/today
 *
 * @param {Object} data
 * @param {number} data.driverUserId
 * @returns {Promise<{ result: Object }>} result.data = { routeDate, planId, planStatus, driverRoute, tasks }
 */
export const getDriverDeliveryToday = function (data) {
  return wrapRequest({
    url: BASE + 'driver/delivery/today',
    method: 'GET',
    data: {
      driverUserId: data.driverUserId
    }
