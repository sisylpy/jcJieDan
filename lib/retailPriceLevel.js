...
/**
 * 订货规格名是否等于大包装单位名（优先于基本单位判断，避免二者同名时误用 nxDgWillPrice）
 */
...
/**
 * 与弹窗单价一致：先按大包装名取 nxDgWillPriceTwo，再按基本单位取 one / nxDgWillPrice
...
export function getRetailWillPriceForStandard(disGoodsEntity, standardName) {
  ...
  const snap = {
    ...
    will: disGoodsEntity.nxDgWillPrice,
  };
  ...
  if (stdT === baseT) {
    ...
    const w = disGoodsEntity.nxDgWillPrice;
    ...
    if (!isPlaceholderWillPrice(w)) {
      dbg("getRetailWillPriceForStandard", {
        branch: "base-unit-nxDgWillPrice",
        ...snap,
        out: w,
      });
      return w;
...
export function getRetailBuyingPriceForStandard(disGoodsEntity, standardName) {
  ...
  const b = disGoodsEntity.nxDgBuyingPrice;
  const out =
