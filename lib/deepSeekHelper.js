...
async function optimizeTextWithDeepSeek(text, options = {}) {
  ...
  try {
    console.log(`${logPrefix} 开始调用 DeepSeek API，输入文本:`, text, '温度:', temperature);
    ...
    return new Promise((resolve, reject) => {
      ...
      wx.request({
        ...
        success: (res) => {
          console.log(`${logPrefix} API 响应成功:`, res);
          
          if (res.statusCode !== 200) {
            console.error(`${logPrefix} API 返回非 200 状态码:`, res.statusCode, res.data);
            reject(new Error(`API 请求失败，状态码: ${res.statusCode}`));
            return;
          }
          
          if (!res.data || !res.data.choices || !res.data.choices[0]) {
            console.error(`${logPrefix} API 响应格式不正确:`, res.data);
            reject(new Error('API 响应格式不正确'));
            return;
          }
          
          if (res.data.error) {
            console.error(`${logPrefix} API 返回错误:`, res.data.error);
            reject(new Error(res.data.error.message || 'API 返回错误'));
            return;
          }
          
          const optimizedText = res.data.choices[0].message.content;
          console.log(`${logPrefix} 优化后的文本:`, optimizedText);
          resolve(optimizedText);
        },
        fail: (err) => {
          console.error(`${logPrefix} API 请求失败:`, err);
          reject(new Error('API 请求失败: ' + JSON.stringify(err)));
        }
      });
    });
  } catch (error) {
    console.error(`${logPrefix} DeepSeek API 调用错误:`, error);
...
async function detectBusinessType(text, options = {}) {
  ...
  try {
    console.log(`${logPrefix} 开始判断业务类型，输入文本:`, text);
    if (originalOrder) {
      console.log(`${logPrefix} 原订单信息:`, originalOrder);
    }
    ...
    try {
      ...
      console.log(`${logPrefix} 业务类型判断结果:`, action);
      
      // 如果是修改订单且有 updatedOrder，返回修改后的订单
      if (action === 'modify' && parsed.updatedOrder) {
        console.log(`${logPrefix} 修改后的订单:`, parsed.updatedOrder);
        return { 
          action, 
      ...
      console.error(`${logPrefix} 解析业务类型失败:`, e, '原始结果:', result);
      // 默认返回修改
      return { action: 'modify' };
    }
  } catch (error) {
    console.error(`${logPrefix} 业务类型判断失败:`, error);
    // 默认返回修改
    return { action: 'modify' };
  }
}
