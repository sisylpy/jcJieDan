...
Page({
  ...
  _getGoodsList() {
    ...
    getNxPurGoodsDetailList(data)
      .then(res => {
        load.hideLoading();
        if (res.result.code == 0) {
          console.log("========== nx 项目采购详情数据结构 ==========");
          ...
          if (res.result.data.arr && res.result.data.arr.length > 0) {
            const purItem = res.result.data.arr[0];
            console.log("arr[0] 完整对象:", JSON.stringify(purItem, null, 2));
            console.log("arr[0] 所有字段:", Object.keys(purItem));
            console.log("采购批次ID字段:", purItem.nxDistributerPurchaseGoodsId);
            console.log("采购日期字段 - nxDpgPurchaseDate:", purItem.nxDpgPurchaseDate);
            console.log("申请日期字段 - nxDpgApplyDate:", purItem.nxDpgApplyDate);
            console.log("时间字段 - nxDpgTime:", purItem.nxDpgTime);
            console.log("采购数量字段 - nxDpgBuyQuantity:", purItem.nxDpgBuyQuantity);
            console.log("采购金额字段 - nxDpgBuySubtotal:", purItem.nxDpgBuySubtotal);
            console.log("采购单价字段 - nxDpgBuyPrice:", purItem.nxDpgBuyPrice);
            console.log("库存相关字段 - nxDisGoodsShelfStockEntities:", purItem.nxDisGoodsShelfStockEntities);
            console.log("采购员实体 - nxPurchaserEntity:", purItem.nxPurchaserEntity);
            console.log("供货商实体 - jrdhSupplierEntity:", purItem.jrdhSupplierEntity);
            
            if (purItem.nxDisGoodsShelfStockEntities && purItem.nxDisGoodsShelfStockEntities.length > 0) {
              const stockItem = purItem.nxDisGoodsShelfStockEntities[0];
              console.log("库存实体[0] 完整对象:", JSON.stringify(stockItem, null, 2));
              console.log("库存实体[0] 所有字段:", Object.keys(stockItem));
            }
          }
          
          if (res.result.data.itemList && res.result.data.itemList.length > 0) {
            console.log("itemList[0]:", res.result.data.itemList[0]);
          }
          console.log("=============================================");
  ...
  _scrollToTargetPurchase() {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < arr.length; i++) {
        const purId = arr[i].nxDistributerPurchaseGoodsId || arr[i].gbDistributerPurchaseGoodsId;
        if (purId == targetPurGoodsId) {
          targetIndex = i;
          break;
        }
      }
  ...
  init_top_echarts: function (goods) {
    var id = goods.nxDistributerGoodsId || goods.gbDistributerGoodsId;
    var that = this;
  ...
  toFenxi(e) {
    ...
    wx.navigateTo({
      url: '../../../../subPackage-goods/pages/goods/goodsPurList/goodsPurList?disGoodsId=' +  (this.data.disGoods.nxDistributerGoodsId || this.data.disGoods.gbDistributerGoodsId) + '&purGoodsId=' + purId, 
    })
  },
