...
Page({
  data: {
    ...
    windowWidth: 0,
    windowHeight: 0,
    imagePreviewHeight: 0, // 图片区固定高度（px），约 22% 窗口，为订单列表多留空间
    statusBarHeight: 0,
    navBarHeight: 0,
  ...
  onLoad: function (options) {
    ...
    // 图片区占屏比例（改这里即可：0.22=22%，0.18=18%，0.25=25%）
    const imagePreviewHeight = Math.round((systemInfo.windowHeight || 600) * 0.32);
  ...
  onShow: function () {
    // if (this.data.isTTSReading || this.data.isCheckMode) {
    //   const that = this;
    //   setTimeout(function () { that._logLayoutHeights(); }, 300);
    // }
  ...
  showGoodsList(e) {
    ...
    if (agent != '2' && agent !== 2) {
      ...
      // 用户修改了名字、显示了搜索结果或推荐商品，但未选择商品就关闭时，恢复显示原文 nxDoGoodsOriginalName
      ...
      if (hadSearchOrRecommend && userDidNotSelect && order?.nxDoGoodsOriginalName != null) {
        updateData["orderArr[" + index + "].nxDoGoodsName"] = order.nxDoGoodsOriginalName;
      }
      this.setData(updateData);
    }
    this.setData({
  ...
  _choiceGoods: function (onSuccess) {
    ...
    // 
    // if(orderToSend.nxDoStandard == '个'){
    //   orderToSend.nxDoStandard = this.data.printStandard;
    // } 
