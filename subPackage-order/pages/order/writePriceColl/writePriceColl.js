...
Page({
  ...
  inputValue(e) {
    ...
    if (value <= 9 && value >= 0) {
      ...
      if (this.data.depHasSubs > 0) {
        oldValue = this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex].nxDoPrice;
      } else {
      ...
      if (this.data.depHasSubs > 0) {
        this.setData({
          ["applyArr[" + this.data.focusParentIndex + "].depOrders[" + this.data.focusIndex + "].nxDoPrice"]: newValue,
        })
      } else {
        this.setData({
      ...
      if (value == "del") {
        ...
        if (newValue.length > 0) {
          ...
          if (this.data.depHasSubs > 0) {
            this.setData({
              ["applyArr[" + this.data.focusParentIndex + "].depOrders[" + this.data.focusIndex + "].nxDoPrice"]: "",
            })
          } else {
            this.setData({
      ...
      if (value == "close") {
        if (this.data.depHasSubs > 0) {
          if (this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex].nxDoPrice !== null) {
            this._save();
          }
          this.setData({
            focusParentIndex: -1,
      ...
      if (value == "next") {
        if (this.data.depHasSubs > 0) {
          ...
          if (focusIndex !== this.data.applyArr[this.data.focusParentIndex].depOrders.length - 1) {
            ...
            if (focusParentIndex !== this.data.applyArr.length - 1) {
              ...
              if (this.data.applyArr[depIndex].depOrders.length > 0) {
                this.setData({
  ...
  changeFocusIndexPrice(e) {
    ...
    if (this.data.depHasSubs > 0) {
      if (this.data.focusIndex !== -1 && this.data.focusParentIndex !== -1) {
        var itemPrice = this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex].nxDoPrice;
  ...
  quickPriceDep(e) {
    ...
    if (this.data.focusParentIndex !== -1 && this.data.focusIndex !== -1 && this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex].nxDoPrice !== null) {
      this._save();
    }
    var index = e.currentTarget.dataset.index;
    var parentIndex = e.currentTarget.dataset.depindex;
    var orderData = this.data.applyArr[parentIndex].depOrders[index].nxDepartmentDisGoodsEntity.nxDdgOrderPrice;
    var orderPrice = "applyArr[" + parentIndex + "].depOrders[" + index + "].nxDoPrice";
    this.setData({
  ...
  _savePriceOrder(e) {
    ...
    var itemOrderPrice = ""
    if (this.data.depHasSubs > 0) {
      itemOrder = this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex];
      itemOrderPrice = this.data.applyArr[this.data.focusParentIndex].depOrders[this.data.focusIndex].nxDoPrice;
    ...
    if (itemOrderPrice !== null && itemOrderPrice > 0) {
      ...
      giveOrderPriceColl(data).then(res => {
        load.hideLoading();
        if (res.result.code == 0) {
          this._initData()
        }else{
