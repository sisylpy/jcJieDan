...
Page({
  ...
  _saveFillOrder: function (e) {
    ...
    var dg = {
      ...
      nxDoGoodsOriginalName: this.data.itemDis.nxDgGoodsName,
    ...
    if (this.data.beforeId !== '-1') {
      console.log('使用 saveOrderBeforeTask');
      saveOrderBeforeTask(dg).then(res => {
        console.log('saveOrderBeforeTask 返回:', res.result.code);
        if (res.result.code == 0) {
          ...
          console.log('saveOrderBeforeTask - fromOcrOrder:', this.data.fromOcrOrder, 'fromStorage:', wx.getStorageSync('fromOcrOrder'), '最终判断:', fromOcrOrder2);
          if (fromOcrOrder2) {
            ...
            console.log('saveOrderBeforeTask - isOcrOrderPage:', isOcrOrderPage2);
            ...
            if (isOcrOrderPage2) {
              ...
              // 更新 ocrOrder 页面的订单列表及分页
              var orderArr = prevPage.data.orderArr || [];
              orderArr.splice(insertIndex, 0, newOrder);
              var newTotalOrders = (prevPage.data.totalOrders || 0) + 1;
              var pageSize = prevPage.data.pageSize || 10;
  ...
  _saveOrderCash: function (e) {
    ...
    var that = this;
    if (this.data.beforeId !== '-1') {
      saveCashBeforeTask(dg).then(res => {
        if (res.result.code == 0) {
          // 如果是从 ocrOrder 页面跳转过来的，需要特殊处理
          // 同时检查 URL 参数和 storage 标记
          var fromOcrOrder2 = this.data.fromOcrOrder === '1' || this.data.fromOcrOrder === 1 || wx.getStorageSync('fromOcrOrder') === '1';
