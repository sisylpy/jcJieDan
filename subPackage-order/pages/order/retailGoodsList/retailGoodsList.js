...
Page({
  ...
  data: {
    ...
    /** 采购/上游用户 id；saveCashBefore 用，非 options 传入则 -1 走 saveCash */
    beforeId: '-1',
  },
  ...
  onLoad: function (options) {
    ...
    const beforeIdFromOpt =
      options &&
      options.beforeId != null &&
      String(options.beforeId).trim() !== ''
    ...
    this.setData({
      ...
      beforeId: beforeIdFromOpt,
    })
    
    saveRetailDepartment(this.data.disId).then(res =>{
      if(res.result.code == 0){
        this.setData({
          retailDep: res.result.data,
  ...
  _doSearch(searchStr) {
    ...
    queryDisGoodsByQuickSearchCollDis(data).then(res => {
      // 竞态处理：只处理最新请求的响应
  ...
  applyGoods(e) {
    ...
    this.setData({
      ...
      level: 1,
      printStandard: item.nxDgGoodsStandardname,
      applyNumber: "",
      applyRemark: "",
    });
  },
  ...
  _setEditApplyData: function(applyItem, itemDisData) {
    const std = applyItem.nxDoStandard || '';
    const pl = itemDisData ? resolveNxDoCostPriceLevel(itemDisData, std) : 1;
    this.setData({
      showCash: true,
      showOperation: false,
  ...
  /**
   * 零售列表（depSettleType=2）：仅 saveCash / updateOrder 现金口径
   */
  onRetailModalConfirm(e) {
    const edit = this.data.editApply;
    console.log("[retailGoodsList] onRetailModalConfirm", {
      editApply: edit,
      detail: e.detail,
    });
    if (edit) {
  ...
  changeStandard: function (e) {
    ...
    const lv =
      e.detail.level != null && e.detail.level !== ""
        ...
        : dis
          ...
          : 1;
    const printStandard =
      lv === 2 && dis && dis.nxDgWillPriceTwoStandard
    ...
    console.log("[retailGoodsList] changeStandard", {
      applyStandardName: std,
      levelFromModal: e.detail.level,
      resolvedLevel: lv,
      printStandard,
      nxDgWillPriceTwoStandard: dis && dis.nxDgWillPriceTwoStandard,
  ...
  _updateDisOrderCash(e) {
    ...
    console.log("[retailGoodsList] _updateDisOrderCash updateOrder", dg, {
      isBigPackStandardName: dis && std ? isBigPackStandardName(dis, std) : false,
    });
  ...
  _saveOrderCash: function (e) {
    ...
    if (isSentinelPrice01(price)) {
      console.warn(
        "[retailGoodsList] _saveOrderCash 零售价占位 0.1，nxDoPrice 置为 0",
        { rawPrice: price, std },
      );
    }

    console.log("[retailGoodsList] _saveOrderCash 计价", {
      nxDistributerGoodsId: dis && dis.nxDistributerGoodsId,
      nxDgGoodsName: dis && dis.nxDgGoodsName,
      applyStandardName: std,
      nxDgGoodsStandardname: dis && dis.nxDgGoodsStandardname,
      nxDgWillPriceTwoStandard: dis && dis.nxDgWillPriceTwoStandard,
      nxDgWillPriceTwo: dis && dis.nxDgWillPriceTwo,
      nxDgWillPriceOne: dis && dis.nxDgWillPriceOne,
      nxDgWillPrice: dis && dis.nxDgWillPrice,
      isBigPackStandardName: dis && std ? isBigPackStandardName(dis, std) : false,
      nxDoCostPriceLevel: pl,
      resolvedWillPrice: price,
      priceForApi,
      costForApi,
      costPriceResolved: costPrice,
      nxDgBuyingPriceFallback: dis && dis.nxDgBuyingPrice,
    ...
    console.log("[retailGoodsList] _saveOrderCash saveCash 请求体 dg", dg);
    var that = this;
