...
Page({
  ...
  _initData() {
    ...
    // 始终使用原始接口获取数据
    getOrderPageWithTraceReport(data)
      .then(res => {
        load.hideLoading();
  ...
  selectDepartment(e) {
    ...
    if (this.data.openType == 'paste') {
      ...
      wx.navigateTo({
        url: '/subPackage-order/pages/order/ocrOrder/ocrOrder?depFatherId=' + this.data.depFatherId +
          '&depId=' + this.data.depId + '&depName=' + depName,
      })
    } else {
      wx.navigateTo({
  ...
  recognizeOrder(e) {
    ...
    if (hasSubDepartments) {
      ...
      const url = '/subPackage-order/pages/order/ocrOrder/ocrOrder' +
        '?depFatherId=' + depFatherId +
        '&depId=' + depId +
      ...
      wx.navigateTo({
        ...
        fail: (err) => {
          ...
          wx.navigateTo({
            url: '/subPackage-order/pages/order/ocrOrder/ocrOrder',
            success: (testRes) => {
  ...
  showSuyuan(e) {
    const order = e.currentTarget.dataset.order;
    if (!order || !order.nxTraceReportEntity) {
      wx.showToast({
        title: '暂无溯源报告',
        icon: 'none'
      });
    ...
    const traceReport = order.nxTraceReportEntity;
