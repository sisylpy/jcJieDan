...
test('purchase finance overview separates current obligations from payment period', () => {
  ...
  assert.match(logic, /companyDirectOutstandingAmount/)
  ...
  assert.match(view, /公司直付待付款/)
  assert.match(view, /companyDirectOutstandingCount/)
  assert.match(view, /全部历史未结事实/)
  assert.match(view, /按实际付款日期筛选/)
  assert.doesNotMatch(view, /资金总览.*pm-grid/)
})

test('company direct funding keeps classification and actual payment visibly separate', () => {
  const logic = read('subPackage/pages/management/purchaseManagement/fundingClassification/fundingClassification.js')
  const view = read('subPackage/pages/management/purchaseManagement/fundingClassification/fundingClassification.wxml')
  assert.match(logic, /COMPANY_DIRECT_PURCHASE/)
  assert.match(logic, /directRemainingAmount/)
  assert.match(view, /item\.goodsName/)
  assert.match(view, /已付.*directPaidAmount/)
  assert.match(view, /待付.*directRemainingAmount/)
  assert.match(view, /不会自动发起微信或银行转账/)
  assert.match(view, /登记实际付款/)
})
